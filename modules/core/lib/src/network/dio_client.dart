...
/// Class for managing Dio HTTP client with automatic token refresh and retry logic.
class DioClient {
  DioClient(this.instance, this._sessionManager) {
  ...
  Future<void> _onRequest(RequestOptions options, RequestInterceptorHandler handler) async {
    final session = await _sessionManager.session;
    if (session != null) {
  ...
  Future<void> _handleTokenRefresh(DioException error, ErrorInterceptorHandler handler, SessionModel session) async {
    ...
    await _refreshLock.synchronized(() async {
      final currentSession = await _sessionManager.session;
